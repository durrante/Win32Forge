...
function New-AppDocumentation {
    ...
    $description = if ($AppConfig.Description -and ([string]$AppConfig.Description).Trim()) { ([string]$AppConfig.Description).Trim() } else { '_No description provided_' }
    ...
    $rcSection = ''
    $rcList = @($AppConfig.ReturnCodes | Where-Object { $_ })
...
"@
    ...
    $toolVer   = if ($global:Win32ForgeVersion) { " v$($global:Win32ForgeVersion)" } else { '' }
...
> Generated by **[Win32Forge](https://github.com/durrante/Win32Forge)**${toolVer} on ${timestamp}
>
